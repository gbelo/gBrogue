...
//	short probabilities[13] =						{50,	42,		52,		3,		0,		7,		8,		2,		3,      3,        0,		0,		0};
	short probabilities[13] =						{50,	47,		45,		2,		0,		6,		6,		2,		3,      2,        0,		0,		0};
...
item *makeItemInto(item *theItem, unsigned long itemCategory, short itemKind) {
	...
	switch (itemCategory) {
		...
		case WEAPON:
			...
			if (itemKind == DART || itemKind == INCENDIARY_DART || itemKind == POISON_DART || itemKind == TRANQUILIZER_DART || itemKind == JAVELIN) {
...
void populateItems(short upstairsX, short upstairsY) {
	...
	if (rogue.depthLevel > AMULET_LEVEL) {
		...
		if (rogue.depthLevel == BIG_LEVEL || rogue.depthLevel == NARROW_LEVEL) // rewards on special floors! -- gsr
...
boolean hitMonsterWithProjectileWeapon(creature *thrower, creature *monst, item *theItem) {
	...
	if (thrownWeaponHit) {
		...
		// Weapons don't always break when thrown now -- gsr
		if (theItem->kind == DART || theItem->kind == POISON_DART || theItem->kind == TRANQUILIZER_DART || theItem->kind == JAVELIN)
            return true;
        else
            return false;
	} else {
...
/*			if (theItem->kind == POTION_HALLUCINATION && (theItem->flags & ITEM_MAGIC_DETECTED)) {
        ...
        if (monst = monsterAtLoc(x, y) && monst->creatureState != MONSTER_ALLY && monst->creatureState == MONSTER_SLEEPING)
            alertMonster(monst);
...
//	maxDistance = (12 + 2 * max(rogue.strength - player.weaknessAmount - 12, 2));
	// gsr
	maxDistance = max(4, ringPropulsionBonus(rogue.throwingBonus) + 0.75 * (rogue.strength - player.weaknessAmount));
...
/*		case SCROLL_PROTECT_ARMOR:
		...
		case SCROLL_GREAT_IDENTIFY:
			updateIdentifiableItems();
			if (!(theItem->flags & ITEM_IDENTIFIED))
                messageWithColor("this is a scroll of great identify.", &itemMessageColor, true);
			identify(theItem);
			if (numberOfMatchingPackItems(ALL_ITEMS, ITEM_CAN_BE_IDENTIFIED, 0, false) == 0) {
				message("everything in your pack is already identified.", false);
				break;
...
/*		case POTION_DESCENT:
		...
		case POTION_VITALITY:
			...
			break;
/*		case POTION_CAUSTIC:
			spawnDungeonFeature(player.xLoc, player.yLoc, &dungeonFeatureCatalog[DF_CAUSTIC_GAS_CLOUD_POTION], true, false);

...
void addXPXPToAlly(short XPXP, creature *monst) {
    ...
    // We're only loosely allied with the player
...
//        && !(monst->bookkeepingFlags & MB_TELEPATHICALLY_REVEALED)
        ...
        // Level up!
        if (monst->xpxp >= XPXP_NEEDED_FOR_EMPOWERMENT * (monst->totalPowerCount+1)) // xpxp level system -- gsr
        {
...
//    return (theItem->kind == STAFF_BLINKING || theItem->kind == STAFF_OBSTRUCTION || theItem->kind == STAFF_DOMINATION ? 10000 : 5000) / theItem->enchant1;
    if (theItem->kind == STAFF_DOMINATION)
        return 0;
    else if (theItem->kind == STAFF_BLINKING || theItem->kind == STAFF_OBSTRUCTION)
        return 10000 / theItem->enchant1;
    else
        return 5000 / theItem->enchant1;
